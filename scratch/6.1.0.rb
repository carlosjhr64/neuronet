...
module Neuronet
  ...
  class FeedForward < Array
    ...
    ##########################################
    # Added for compatibitily with test_runs #
    ##########################################
    alias number num
  ...
  class ScaledNetwork < FeedForward
    ...
    ##########################################
    def pairs(pairs)
      pairs.shuffle.each{|input, target| set(input); train!(target)}
      if block_given?
        while yield
          pairs.shuffle.each{|input, target| set(input); train!(target)}
        end
  ...
  module NeoYinYang
    def self.[](size)
      Neo.bless YinYang.bless ScaledNetwork.new [size, size, size, size]
    end
  end
end
