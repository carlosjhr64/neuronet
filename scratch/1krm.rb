...
module Neuronet
  ...
  class Neuron < Node
    ...
    def mue
      # mue is entirely based on a sum of external activations.
      # In a classic feed foward,
      # it can be considered a propety of the next layer.
      1.0 + @connections.sum{|connection| connection.node.activation}
    end

    def kappa
      # kappa is based on the neuron's connnection weights.
      # No two neurons can be expected to have the same kappa value.
      @connections.sum do |connection|
        a = connection.node.activation
        connection.weight * a * (1.0 - a)
      end
    end
  ...
  class FeedForward < Array
    ...
    def rho!
      @rho = Math.sqrt(self.sum{|layer| l=layer.length; l*l} / self.length)
    end
    def rho
      @rho || rho!
    end
    ...
    def nu!
      @nu = self.sum{|layer| layer.length}
    end
    def nu
      @nu || nu!
    end

    # root mean square kappa value
    def kappa
      Math.sqrt( self.sum{|layer| layer.sum{|node| k=node.kappa; k*k}} / nu )
    end

    # list of layer mue values
    def mues
      self.map{|layer| layer.first.mue}
    end

    # Let me just muk something up!
    def muk
      k = 1+kappa*rho
      mues.sum{|mue| k*mue}
    end
    ...
    def train(target)
      @salida.train(target, @learning/muk)
      self
    end
